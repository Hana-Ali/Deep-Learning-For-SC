...
def check_missing_mrtrix_cleaning(NEEDED_FILE_PATHS, MAIN_MRTRIX_PATH):
    ...
    # Define variable that stores whether or not we should do MRtrix cleaning
...
def check_missing_mrtrix_registration(NEEDED_FILE_PATHS, MAIN_MRTRIX_PATH, ATLAS, T1_REG_FOLDER_NAME, ATLAS_REG_FOLDER_NAME):
    ...
    # Get the folder for this specific atlas
    ATLAS_NAME = ATLAS.split(os.sep)[-1].split(".")[0]
    ATLAS_FOLDER = os.path.join(ATLAS_REG_FOLDER_NAME, ATLAS_NAME)
    
    # Grab all the mif files in the atlas registration folder
    MRTRIX_ATLAS_REG_MIF_FILES = glob_files(ATLAS_FOLDER, "mif")
    ...
    if MRTRIX_REGISTRATION:
        ...
        check_output_folders(ATLAS_FOLDER, "MRtrix atlas registration folder", wipe=True)
...
def check_missing_mrtrix_connectome(NEEDED_FILE_PATHS, MAIN_MRTRIX_PATH, CONNECTIVITY_FOLDER_NAME, ATLAS):
    ...
    # Define the connectivity file for this specific atlas
    ATLAS_NAME = ATLAS.split(os.sep)[-1].split(".")[0]
    CONNECTIVITY_ATLAS_FOLDER = os.path.join(CONNECTIVITY_FOLDER_NAME, ATLAS_NAME)
    
    # Grab all the csv files
    MRTRIX_CONNECTOME_CSV_FILES = glob_files(CONNECTIVITY_ATLAS_FOLDER, "csv")
    ...
    if MRTRIX_CONNECTOME:
        print("--- MRtrix atlas connectome files not found. Cleaning MRtrix connectome folder.")
        check_output_folders(CONNECTIVITY_ATLAS_FOLDER, "MRtrix atlas connectome folder", wipe=True)
