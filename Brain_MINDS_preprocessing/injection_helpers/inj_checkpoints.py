...
def not_done_yet_injection_roi_tckstats(REGION_ID, ATLAS_STPT, INJECTION_ROI_TRACTS_STATS_FOLDER, TYPE="includes_both"):
    ...
    for idx, roi_mif_path in enumerate(INDIVIDUAL_ROIS_MIF_PATHS):
        ...
        roi_name = roi_mif_path.split("/")[-1]
        # Get the injection ROI tracts and stats path
        (INJECTION_ROI_TRACTS_STATS_FOLDER_SPECIFIC, INJECTION_ROI_LENGTHS_PATH, 
         INJECTION_ROI_COUNT_PATH, INJECTION_ROI_MEAN_PATH, INJECTION_ROI_MEDIAN_PATH,
         INJECTION_ROI_STD_PATH, INJECTION_ROI_MIN_PATH, INJECTION_ROI_MAX_PATH) = get_injection_roi_tracts_stats_path(REGION_ID, 
                                                                                                                       roi_name, 
                                                                                                                       TYPE)
