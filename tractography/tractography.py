...
if not os.path.exists(ATLAS_FOLDER):
    print("Atlas folder not found. Created folder: " + ATLAS_FOLDER)
...
# For the output file - create if not
if not os.path.exists(DWI_OUTPUT_FOLDER):
    print("SRC folder not found. Created folder: " + DWI_OUTPUT_FOLDER)
...
PEDRO_RECONSTRUCTION_DTI = "dsi_studio --action=rec --source=dwi_clean.src.gz --method=1 --record_odf=1 \
      --param0=1.25 --motion_correction=0 --output=dwi_clean_dti.fib.gz"
PEDRO_EXPORT = "dsi_studio --action=exp --source=dwi_clean_dti.fib.gz --export=fa"
PEDRO_RECONSTRUCTION_QSDR = "dsi_studio --action=rec --source=dwi_clean.src.gz --method=7 --record_odf=1 \
...
EXPORTING = "dsi_studio --action=exp --source=dwi_clean_qsdr.fib.gz --export=qa,rdi,fa,md"
# Calling the subprocess
subprocess.call(EXPORTING, shell=True)

# --------------- Tractography reconstruction command --------------- #
TRACTOGRAPHY = "dsi_studio --action=trk --source=dwi_clean_qsdr.fib.gz --fiber_count=1000000 --output=no_file \
    --method=0 --interpolation=0 --max_length=400 --min_length=10 --otsu_threshold=0.6 --random_seed=0 --turning_angle=55 \
        --smoothing=0 --step_size=1 --connectivity=aal116_mni.nii,schaefer100_mni.nii.gz --connectivity_type=end \
            --connectivity_value=count --connectivity_threshold=0.001"
# Calling the subprocess
...
for tract in glob.glob("{}/*trk.gz".format(TRACT_FOLDER)):
    ...
    CLEANING = "dsi_studio --action=ana --source=dwi_clean_qsdr.fib.gz --tract={} --export=stat".format(tract_filename)
    # Calling the subprocess
