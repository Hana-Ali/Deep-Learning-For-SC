...
class ChannelAttention(nn.Module):
    ...
    def forward(self, x):
        # x is of shape (batch_size, time_steps, channels)
        x = torch.tanh(self.fc1(x))
        attention_weights = F.softmax(self.fc2(x), dim=2)
...
class ConvLSTM(nn.Module):
    ...
    def forward(self, x):
        ...
        for t in range(time_steps):
            ...
            attention_weights = self.attention(x_t.view(batch_size, -1, channels)) # shape (batch_size, 1, channels)
            x_t = torch.sum(x_t * attention_weights, dim=2) # weighted sum along channels axis
