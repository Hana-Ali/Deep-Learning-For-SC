...
parser.add_argument("-b", "--batch_size", help="batchsize",
			default=32,
			type=int)
parser.add_argument("-enc", "--encoder", help="encoder or not",
                    default="",
                    type=str)
...
if encoder != "":
    pred_name = pred_name + "_" + encoder
    train_name = train_name + "_" + encoder
...
encoder_folder = os.path.join(main_logs_path, "encoders", encoder)
...
check_output_folders(encoder_folder, "encoder_folder", wipe=False)
...
encoder_filename = os.path.join(encoder_folder, "resnet_streamlines.h5")
...
configs = {
    ...
    ####### Encoder stuff #######
    "two_models" : True,
    "encoder_choice" : encoder, # Encoder name
    "encoder_filename" : encoder_filename, # Filename of encoder
