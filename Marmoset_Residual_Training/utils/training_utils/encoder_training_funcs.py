...
def encoder_train_loop_nodes(train_loader, encoder, model, criterion, optimizer, epoch, streamline_arrays_path, separate_hemisphere,
...
def encoder_val_loop_nodes(val_loader, encoder, model, criterion, epoch, streamline_arrays_path, separate_hemisphere,
...
# Define the function to get the encoder output
def encoder_pass(encoder, wmfod_cube, previous_predictions, training_task="classification", n_gpus=None, distributed=False, use_amp=False):
    ...
    if n_gpus:
        ...
        if not distributed:
            ...
            if n_gpus == 1:
                ...
                # Get the encoder on the GPU
                encoder = encoder.cuda()
        
    # Compute the output
    if "autoencoder" in str(encoder.__class__.__name__).lower():
        embedding = encoder.forward_encoder(wmfod_cube)
    else:
        embedding = encoder(wmfod_cube, previous_predictions)
